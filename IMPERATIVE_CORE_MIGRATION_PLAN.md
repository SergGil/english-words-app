...
- **[ ] Залишок** (~17 `window.*`, переважно читачі — `i18n.ts`/
  `sidebar.ts`): `checkAchievements`, `speak`, `_renderVoices`,
  `openLearningPath`, `renderLearningPath`, `_refreshRangeOptions`,
  `invalidateCatCache`, `_renderWeakWords`, `invalidateReadingIndex`,
  `openGrammarContent`, `_refreshIdiomsUI`, `_refreshPrefetchUI`,
  `_refreshPixabayStatus`, `_refreshCloudSyncUI`, `_refreshNotifUI`,
  `renderStats`, `_refreshStatsExtras`, `_speakWithLang` — типово
  цикл-чутливі (i18n.ts/sidebar.ts імпортуються майже всюди), потребують
  динамічних імпортів за зразком Фази 7.5. Окрема майбутня робота.
